{ lib, stdenv, fetchurl, bootstrap ? true, texinfo4 }:
...
let
  extraArgs = {
    ...
    postBuild = lib.optionalString (!bootstrap) ''
...
in stdenv.mkDerivation (rec {
  ...
  patches = [
    ...
    (fetchurl {
      url =
        "https://sources.debian.org/data/main/m/m4/1.4.18-2/debian/patches/01-fix-ftbfs-with-glibc-2.28.patch";
...
} // (if bootstrap then { } else extraArgs))
